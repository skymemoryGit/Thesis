...
def genera_set_train_val_test_Con_Correlated_Feature(colonna_target,ls): #viene passato il target e le best feature coorelate 
    ...
    Y_train = train_data[:m_train,0]   #voglio predire target che è in poszione 0 da come ho costruito io
    print("target",Y_train)
...
def turn_in_number_from_gender(data):
    #print("sono in def")
    #print("le immagine unique sono",data["What is your Gender? "].unique())
    data['What is your Gender? '] = data['What is your Gender? '].map({'Male': 1, "Female":0,"non-binary":2},)
...
if __name__ == '__main__':
    ...
    print("\ntrasformo M F in 0 1 ")
    turn_in_number_from_gender(df_start)
    #eliminare le persone che sono di sesso non-binary
    print("done\n")


    print("le immagine unique PRIMA DEL REMOVE sono",df_start["What is your Gender? "].unique())
    print(len(df_start))
    ls_user=df_start["id_owner"].unique()
    print("In totale ci sono user distinti",len(ls_user))
    
    df_start.drop(df_start[df_start['What is your Gender? '] == 2].index, inplace = True)
    #print(df["What is your Gender? "])
    
    
    print("le immagine unique DOPO REMOVE sono",df_start["What is your Gender? "].unique())
    print(len(df_start))
    ls_user=df_start["id_owner"].unique()
    print("In totale ci sono user distinti",len(ls_user))
    ...
    print("\nNumber User for training and deciding parameters:",n_user_train)
    ...
    X_train, Y_train, X_val, Y_val, X_test, Y_test = genera_set_train_val_test("What is your Gender? ")
    #avere qualche statistica
    get_statistica_set(Y_train)
    
    #print(X_train[1000:1001])
    #print(Y_train[1000:1001])
    ...
    search.fit(X_train_val_scaled[0:2000], Y_train_val[0:2000])    #per ora metti 500 elemeti, da mettere tutto cmq
    ...
    search = GridSearchCV(clf, param_grid, scoring='f1_macro', cv=8, refit=True, n_jobs = 8)
    search.fit(X_train_val_scaled[0:2000], Y_train_val[0:2000]) 
    y_test_pred = search.predict(Xtest_scaled)
    score = f1_score(Y_test, y_test_pred, average = 'macro')
    print("\nSVM",score)
    ...
    best_feature=['numero_brani', 'anno_max', 'average_anno_publicazione',
           'std_anno_pubblicazione', 'n_brani_solo_perform',
           'n_brani_COLLAB_perform', 'un artista compare multi time:', 'max_time',
           'simpson_index', 
           'N artisti non popular(fans <1000)', 'avg_popularity_songs',
           'std_popularity_songs', 'Is playlist updated', 'perc_pop',
           'perc_hiphop', 'perc_rap', 'perc_rock', 'perc_edm',
           'perc_indie', 'perc_classic', 'perc_kpop', 'perc_metal', 'perc_country',
           'danceability', 'std_danceability', 'energy', 'energy_std', 'loudness',
           'loudness_std', 'speechiness', 'speechiness_std', 'acoutsticness',
           'acoutsticness_std', 'instrumentalness', 'instrumentalness_std',
           'liveness', 'liveness_std', 'valence', 'tempo',
           ]
    X_train, Y_train, X_val, Y_val, X_test, Y_test = genera_set_train_val_test_Con_Correlated_Feature("What is your Gender? ",best_feature)
